    ...
    def __init__(self, file):
        ...
        for idx, row in excel_data.iloc[dialogue_start_idx:].iterrows():
            if not pd.isna(row['Unnamed: 5']):  # 确保'bot'（假设第6列）的列数据不为空
                dialogue_entry = {
                    "scene_definition": row['Unnamed: 1'],  # 场景定义 (假设为第1列)
                    "sequence_number": row['Unnamed: 2'],  # 序号 (假设为第2列)
                    "user_hardware_input": row['Unnamed: 3'],  # 用户硬件输入 (假设为第3列)
                    "user_input": row['Unnamed: 4'],  # 用户输入 (假设为第4列)
                    "bot_response": row['Unnamed: 5'],  # 机器人回复 (假设为第5列)
                    "hardware_output": row['Unnamed: 6'],  # 硬件输出（可选） (假设为第6列)
